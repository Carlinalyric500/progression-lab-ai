...
export const SubmitSuccess: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const suggestionHeadings = await canvas.findAllByText('Next chord suggestions');
    await expect(suggestionHeadings.length).toBeGreaterThan(0);
  },
};
...
export const SubmitLoading: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByText('Generating suggestions...');
  },
};
...
export const SubmitError: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const errorMessages = await canvas.findAllByText('Could not generate suggestions.');
